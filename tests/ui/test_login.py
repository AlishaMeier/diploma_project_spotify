...
ERROR_INVALID_CREDS_RU = "Неверное имя пользователя или пароль."
...
INVALID_LOGIN_DATA = [
    ...
    ("nonexistent.user@mail.com", "любой_пароль", ERROR_NOT_LINKED),
]
...
def test_invalid_password(credentials, login_page: LoginPage, navigation_page: NavigationPage):
    """
    Тест проверяет ошибку неверного пароля.
    Требует прохождения на экран пароля и фиксации ошибки там.
    """
    ...
    with allure.step("3. Проверить отображение системной ошибки"):
        ...
        login_page.should_see_error_message(ERROR_INVALID_CREDS_RU)
...
@pytest.mark.parametrize(
    "username, password, expected_error", INVALID_LOGIN_DATA  # Использует только кейсы 1 и 2
)
